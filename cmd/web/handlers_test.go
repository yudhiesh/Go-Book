...
func TestSignupUser(t *testing.T) {
	...
	tests := []struct {
		...
		{"Short password", "Bob", "bob@example.com", "bob", csrfToken, http.StatusOK, []byte("This field is too short (minimum is 10 characters)")},
...
func TestCreateSnippetForm(t *testing.T) {
	...
	t.Run("Unauthenticated", func(t *testing.T) {
		...
		if statusCode != http.StatusSeeOther {
			t.Errorf("want %d; got %d", http.StatusSeeOther, statusCode)
		}
